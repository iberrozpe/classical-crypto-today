    ...
    sections: [
      ...
      },
      {
        ...
        diagram: {
          ...
          blocks: [
            { label: "FIPS 197 — AES", detail: "The Advanced Encryption Standard itself, published 2001." },
            { label: "FIPS 180-4 / FIPS 202 — SHS & SHA-3", detail: "SHA-2 (180-4) and SHA-3/Keccak (202), the two hash function families in current use." },
            { label: "FIPS 186-5 — Digital Signature Standard", detail: "Specifies DSA, RSA, and ECDSA signatures." },
            { label: "FIPS 203 / 204 / 205 — Post-quantum standards", detail: "ML-KEM (key encapsulation), ML-DSA (signatures), SLH-DSA (hash-based signatures) — finalized August 2024." },
            { label: "SP 800-38 series", detail: "Block cipher modes of operation — SP 800-38D specifically defines GCM." },
            { label: "SP 800-56A / B", detail: "Key-establishment schemes using discrete log (A) and integer factorization/RSA (B) cryptography." },
            { label: "SP 800-90A", detail: "Recommendation for random number generation using deterministic random bit generators (DRBGs) — the standard at the center of the Dual_EC_DRBG story below." },
            { label: "SP 800-57", detail: "Key management guidance, including the key-size/security-level equivalence table this site's Compare page draws from." },
          ],
        },
        practice: [
          {
    ...
    sections: [
      ...
      },
      {
        ...
        diagram: {
          ...
          blocks: [
            { label: "TLS WG", detail: "RFC 8446 (TLS 1.3) and its predecessors — covered in the TLS in Practice module." },
            { label: "LAMPS WG", detail: "RFC 5280 (X.509 PKIX certificate profile) and related PKI specs — covered in Digital Certificates & X.509." },
            { label: "OAuth / JOSE WGs", detail: "RFC 6749 (OAuth 2.0), RFC 7519 (JWT), and the surrounding JOSE (JSON Object Signing and Encryption) specs — covered in JWT & API Auth." },
            { label: "curdle / secsh (historical) WGs", detail: "RFC 4253 and successors defining the SSH transport protocol — covered in the SSH module." },
          ],
        },
        practice: [
          {
      ...
      },
      {
        ...
        diagram: {
          ...
          blocks: [
            ...
            { label: "ISO/IEC 15408", detail: "The Common Criteria for Information Technology Security Evaluation — the framework behind \"Common Criteria certified\" hardware and software." },
            ...
            { label: "ISO/IEC 27001", detail: "Information Security Management Systems (ISMS) — the standard behind the certification most enterprise security questionnaires actually ask for." },
          ],
        },
        practice: [
          {
    ...
    sections: [
      ...
      },
      {
        ...
        diagram: {
          ...
          blocks: [
            { label: "SAML 2.0", detail: "Ratified as an OASIS Standard in 2005 — the XML-based federated-identity assertion format covered in the federated identity use case." },
            { label: "PKCS#11 (Cryptoki)", detail: "The hardware-token API covered in depth in its own use case — see the next section for how OASIS came to own it." },
            { label: "KMIP (Key Management Interoperability Protocol)", detail: "A network protocol letting an application's key-management requests work identically across HSMs and KMS products from different vendors — see \"Go deeper\" below." },
          ],
        },
      },
      {
