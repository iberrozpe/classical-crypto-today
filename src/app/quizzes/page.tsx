...
export const metadata: Metadata = {
  ...
  description: "Every knowledge-check quiz on the site, in one place — one per Learn module, Use Case, and Standards body.",
};
...
export default function QuizzesPage() {
  ...
  const standardsQuizzesList = standardsBodies
    .map((s) => ({ body: s, quiz: getStandardsQuiz(s.slug) }))
    .filter((sq) => sq.quiz !== undefined);

  const totalQuizzes = moduleQuizzes.length + useCaseQuizzes.length + standardsQuizzesList.length;
  const totalQuestions =
    ...
    standardsQuizzesList.reduce((sum, sq) => sum + sq.quiz!.questions.length, 0);
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16">
      ...
      <p className="mt-4 max-w-2xl text-muted">
        {totalQuizzes} quizzes, {totalQuestions} questions total — one for every Learn module, Use
        Case, and Standards body, each with an explanation for every answer. No login, no
        tracking, just immediate feedback.
      </p>
      ...
      <div className="mt-12 space-y-12">
        ...
        <div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {standardsQuizzesList.map(({ body: s, quiz }) => (
              <Link
                key={s.slug}
                href={`/standards/${s.slug}/quiz`}
                className="flex flex-col rounded-lg border border-border bg-surface p-5 transition hover:border-accent hover:bg-surface-hover"
              >
